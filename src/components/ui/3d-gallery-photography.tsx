...
/**
 * Custom hook: loads textures manually via THREE.TextureLoader.
 * Does NOT use React Suspense (unlike drei's useTexture), so the
 * scene keeps rendering with a dark background while images load.
 */
		...
		urls.forEach((url, i) => {
			loader.load(
				...
				(tex) => {
					...
					loaded[i] = tex;
					// Update state with a new array so React sees the change
					setTextures([...loaded]);
				},
				undefined,
				(err) => {
					console.warn(`[Gallery] Failed to load texture ${i}:`, url, err);
				}
			);
		});
...
function GalleryScene({
	...
	// Manual texture loading — no Suspense, no white screen
	const textures = useManualTextures(imageUrls);
	...
	// Count how many textures have loaded
	const loadedTextures = textures.filter(Boolean);
	...
	return (
		<>
			{planesData.current.map((plane, i) => {
				...
				// Skip planes whose texture hasn't loaded yet
...
function FallbackGallery({ images }: { images: ImageItem[] }) {
	...
	return (
		<div className="flex flex-col items-center justify-center h-full bg-dark-bg p-4">
			<p className="text-gray-400 mb-4">WebGL not supported. Showing image list:</p>
